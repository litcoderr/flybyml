...
class BaseTeacherForce(nn.Module):
    """
    ...
    def __init__(self, args):
        ...
        self.action_regressor = LSTMRegressor(
            args.dsensory+args.dinst+args.daction, 
    ...
    def forward(self, batch, prev_context=None):
        # [b, seq_len, 18]
        inp_t = torch.cat([batch['sensory_observations'], batch['instructions'], batch['prev_actions']], dim=2)
        # [b, seq_len, 10]
...
class AlfredBaselineTeacherForce(LightningModule):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.model = BaseTeacherForce(args.model)
        self.mse = nn.MSELoss()
