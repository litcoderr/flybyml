...
Sparse:
    Accumulates reward in an episode and grants the reward only at the last state of episode.
"""
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.args.train.epoch), desc='epoch'):
            for local_step in tqdm(range(self.args.train.steps_per_epoch), desc='local step'):
                ...
                if episode_ended:
                    self.buf.store(obs.cpu().numpy(), action, ep_ret, value, log_prob)
                else:
                    self.buf.store(obs.cpu().numpy(), action, 0, value, log_prob)
